  ...
  return (
    <main className="h-full flex items-center justify-center gap-8">
      <div className="w-[37rem] border border-gray-100 rounded-md p-8">
          ...
          {steps.map((step, index) => (
            <>
              <StepTitle
                ...
                title={step.title}
              />
              {index < steps.length - 1 && <CaretRight weight="bold" />}
            </>
          ))}
        </header>
        ...
        {currentStep === 1 && <Step1 changeStep={changeStep} />}

        {currentStep === 2 && <Step2 changeStep={changeStep} />}

        {currentStep === 3 && <Step3 changeStep={changeStep} />}
